/**
 * Validadores para datos de experto (número de cuenta, titular)
 */
...
// Simple IBAN validator: move country code to end and compute remainder mod 97
// Reference algorithm from IBAN standard
...
function validateNumeroCuentaByBank(banco, numero) {
  ...
  // Si parece un IBAN (comienza con dos letras y dígitos), validar IBAN
  if (/^[A-Za-z]{2}[0-9A-Za-z]{13,32}$/.test(s)) {
    if (!isValidIBAN(s)) return { valid: false, message: "IBAN inválido." };
    return { valid: true };
  }
  // Otherwise allow alphanumeric 15-34
