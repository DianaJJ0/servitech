...
(async function () {
  ...
  try {
    ...
    const logRes = (await Log.insertOne)
      ? await Log.insertOne({ test: true, ts: new Date() })
      : await Log.create({ test: true, ts: new Date() });
    console.log(
      "log insert result:",
      logRes && (logRes.insertedId || logRes._id) ? "ok" : "unknown"
    );
